...
# Set your IPUMS API key. Replace "YOUR_API_KEY" with your key.
...
# --------------------------------------
# This defines an extract request for IPUMS CPS ASEC data from 1994-Present.
# It now includes YRIMMIG to identify newly arrived immigrants.
...
cps_extract_request <- define_extract_cps(
  description = "Annual Labor Market Data by Immigrant Arrival Cohort",
  samples = paste0("cps", seq(1994, latest_year), "_03s"),
  variables = c("YEAR", "ASECWTH", "NATIVITY", "LABFORCE", "EMPSTAT", "AGE", "SEX", 
                "YRIMMIG") # <-- YRIMMIG is included here
)

# Submit the extract request to the IPUMS API
submitted_extract <- submit_extract(cps_extract_request)

# Wait for the extract to be processed. This may take several minutes.
downloadable_extract <- wait_for_extract(submitted_extract)


# 3. Download the Data
# --------------------
# Download the data and DDI files to a local directory.
# This will overwrite old files if they exist.
...
# ---------------------------------
# Use read_ipums_micro to load the data. 
# This section creates three distinct groups for analysis.
...
annual_data_by_arrival <- cps_data %>%
  ...
  mutate(immigrant_group = case_when(
    ...
    # "Recent Immigrants" are foreign-born who arrived in the *previous* calendar year
    NATIVITY == 5 & YRIMMIG == (YEAR - 1) ~ "Recent Immigrant (Arrived Last Year)",
    # "Prior Immigrants" are all other foreign-born with a valid immigration year
    NATIVITY == 5 & YRIMMIG < (YEAR - 1) & YRIMMIG > 0 ~ "Prior Immigrant",
    # All others are grouped as "Other"
    TRUE ~ "Other"
  )) %>%
  # Keep only our defined groups for the analysis
  ...
  # Calculate weighted sums using the corrected logic
  summarise(
...
# ----------------------------
# Reshape the data for plotting and generate the visualization
...
ggplot(plotting_data, aes(x = year, y = rate, color = group, group = group)) +
  ...
  labs(
    title = "U.S. Labor Market Indicators by Immigrant Arrival Cohort",
